...
def get_confirm(update, context):
    ...
    if not dl:
        query.answer(
            text="This task has been cancelled!", show_alert=True
        )
    ...
    if user_id != listener.message.from_user.id:
        query.answer(
            text="This task is not for you!", show_alert=True
        )
    elif data[1] == "pin":
        ...
        if len(id_) > 20:
            ...
            for f in res:
                if f["selected"] == "false" and ospath.exists(
                    f["path"]
                ):
                    try:
                        remove(f["path"])
...
bts_handler = CallbackQueryHandler(
    get_confirm, pattern="btsel", run_async=True
)
