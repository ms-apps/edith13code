...
class GoogleDriveHelper:
    ...
    def cspeed(self):
        try:
            return self.transferred_size / int(
    ...
    def __switchServiceAccount(self):
        if (
            self.__service_account_index
            == SERVICE_ACCOUNTS_NUMBER - 1
        ):
        ...
        self.__sa_count += 1
        LOGGER.info(
            f"Switching to {self.__service_account_index}.json service account"
        )
        self.__service = self.__authorize()
    ...
    def upload(self, file_name: str):
        ...
        self.__updater = setInterval(
            self.__update_interval, self._progress
        )
        try:
            if ospath.isfile(file_path):
                ...
                if link is None:
                    raise Exception(
                        "Upload has been manually cancelled"
                    )
                LOGGER.info(f"Uploaded To G-Drive: {file_path}")
            else:
                mime_type = "Folder"
                dir_id = self.__create_directory(
                    ospath.basename(ospath.abspath(file_name)),
                    parent_id,
                )
                ...
                link = (
                    f"https://drive.google.com/folderview?id={dir_id}"
                )
                if self.__is_cancelled:
                    return
            ...
            if self.__is_cancelled and not self.__is_errored:
                if mime_type == "Folder":
                    LOGGER.info(
                        "Deleting uploaded data from Drive..."
                    )
    ...
    def __upload_dir(self, input_directory, parent_id):
        ...
        for item in list_dirs:
            ...
            if ospath.isdir(current_file_name):
                ...
                new_id = self.__upload_dir(
                    current_file_name, current_dir_id
                )
    ...
    @retry(
        ...
        stop=stop_after_attempt(3),
        retry=(
            retry_if_exception_type(GCError)
            | retry_if_exception_type(IOError)
        ),
    )
    def __upload_file(
        self, file_path, file_name, mime_type, parent_id
    ):
    ...
    def clone(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                durl = self.__G_DRIVE_DIR_BASE_DOWNLOAD_URL.format(
                    dir_id
                )
                if self.__is_cancelled:
                ...
                durl = self.__G_DRIVE_BASE_DOWNLOAD_URL.format(
                    file.get("id")
                )
                buttons = ButtonMaker()
                buttons.buildbutton("☁️ Drive Link", durl)
    ...
    def __cloneFolder(self, name, local_path, folder_id, parent_id):
        ...
        for file in files:
            if file.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                not file.get("name")
                .lower()
                .endswith(tuple(EXTENSION_FILTER))
            ):
                self.__total_files += 1
    ...
    def drive_list(
        ...
        for index, parent_id in enumerate(DRIVES_IDS):
            ...
            if (
                len(DRIVES_NAMES) > 1
                and DRIVES_NAMES[index] is not None
            ):
                msg += (
                    '<span class="container center rfontsize">'
        ...
        with open(f_name, "w", encoding="utf-8") as f:
            f.write(
                hmtl_content.replace("{fileName}", fileName).replace(
                    "{msg}", msg
                )
            )
        return cap, f_name
    ...
    def helper(self, link):
        ...
        try:
            ...
            name = meta["name"]
            LOGGER.info(
                f"Checking size, this might take a minute: {name}"
            )
